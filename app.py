...
<style>
    ...
    /* Engine Guide */
    .engine-guide {
        font-size: 12px; background: #111; padding: 10px; border-radius: 8px; margin-top: 5px; border: 1px dashed #555; color: #aaa;
    }
...
# [TITLE RESTORED: Rocket Removed, Icons Fixed]
st.markdown("<h1 style='text-align: center; color: #FFD700;'>🐯 호찌 & 🐹 햄찌의 퀀트 대작전</h1>", unsafe_allow_html=True)
...
class SingularityEngine:
    ...
    def _get_hamzzi_msg(self, wr, m, can_buy, target, price):
        intros = [
            "야, 쫄보야? 아직도 안 샀어?", "어머? 이 차트를 보고도 가만히 있어?", 
            "눈 떠! 돈 벌 기회잖아!", "이 좋은 걸 보고만 있을 거야?", 
            "나만 부자된다? 흥!", "지금 안 사면 버스 떠나!"
        ]
        
        logic_good = [
            f"**Hawkes 강도** {m['hawkes']:.2f} 뚫었잖아! 기계들이 미친 듯이 매수하는데 넌 뭐해?",
            f"**Omega** {m['omega']:.1f}Hz로 폭발 직전이라구! 우주 끝까지 갈 기세야!",
            f"**GNN** {m['gnn']:.2f}로 시장 돈 다 빨아들이는 중! 완전 블랙홀이야!"
        ]
        
        action_buy = [
            f"잔말 말고 **시장가**로 **{can_buy}주** 긁어! 늦으면 네 손해야!",
            f"지금 당장 **{can_buy}주** 풀매수해! **{target:,}원** 가면 내가 칭찬해줄게!",
            f"인생 역전 하고 싶지 않아? 눈 딱 감고 질러! 불타기 가즈아!"
        ]
        
        logic_bad = [
            f"으악! **VPIN** {m['vpin']:.2f}야! 설거지 당하고 싶어? 정신 차려!",
            f"**Betti Number** 1 떴어. 차트에 구멍 뚫려서 지옥문 열렸다고!",
            f"**Tail Risk** {m['es']:.2f} 실화냐? 한방에 계좌 녹을 수 있어!"
        ]
        
        action_sell = [
            "당장 갖다 버려! 꼴도 보기 싫어! 돔황챠!!",
            "들고 있으면 바보 인증이야. 전량 매도해! 지금 당장!",
            "절대 사지 마. 내 말 안 들으면 나중에 울지도 몰라."
        ]
    ...
    # 🐯 호찌: 꼰대 + 사자성어(설명 포함) + 방어적
    def _get_hojji_msg(self, wr, m, can_buy, target, price):
        idioms_good = [
            "금상첨화(錦上添花, 비단 위에 꽃을 더하다)", 
            "낭중지추(囊中之錐, 주머니 속의 송곳처럼 재능이 드러남)", 
            "파죽지세(破竹之勢, 대나무를 쪼개듯 맹렬한 기세)", 
            "일취월장(日就月將, 나날이 다달이 발전함)"
        ]
        idioms_bad = [
            "사상누각(砂上樓閣, 모래 위에 지은 집)", 
            "내우외환(內憂外患, 안팎으로 근심이 가득함)", 
            "풍전등화(風前燈火, 바람 앞의 등불)", 
        ...
        logic_good = [
            ...
            f"**전이 엔트로피** 흐름이 양호해. 실적과 수급의 조화가 훌륭해.",
        ...
        action_buy = [
            ...
            f"**{target:,}원**까지는 '우보천리(牛步千里)'의 마음으로 진득하게 가져가게나."
        ]
        
        logic_bad = [
            f"**내재 변동성** {m['vol_surf']:.2f} 좀 보게. 위험천만해.",
            f"**Going Concern** 이슈가 있어. 기초가 부실해.",
        ...
        if wr >= 0.70:
            return f"""
            **[🐯 호찌의 훈장님 말씀]**\n
            "{random.choice(intros)} **{sel_idiom_good}**! {random.choice(logic_good)}"\n
            **[⏳ 호찌의 시계열 지침]**\n
            ...
            **👉 {random.choice(action_buy)}**
            ...
            return f"""
            **[🐯 호찌의 신중론]**\n
            "계륵일세. 상승 여력은 있으나 **꼬리 위험(ES)**이 {m['es']:.2f}로 감지돼.
            **거안사위(居安思危, 편안할 때 위태로움을 생각함)**라 했네."\n
            **[⏳ 호찌의 시계열 지침]**\n
            ...
            **👉 유비무환(有備無患)일세. 리스크 관리에 치중하게.**
            ...
            return f"""
            **[🐯 호찌의 불호령]**\n
            "어허! **{sel_idiom_bad}**! {random.choice(logic_bad)} 어디서 이런 걸 가져왔나!"\n
            **[⏳ 호찌의 시계열 지침]**\n
            * 즉시: 관심 종목 삭제.\n* 향후: 쳐다도 보지 말게.\n
            **👉 {random.choice(action_sell)}**
            """
    ...
    def diagnose_portfolio(self, portfolio, cash):
        ...
        h_msgs = [
            f"사장님! 현금 비중이 **{cash_r:.1f}%**야. **[Cash Drag]** 때문에 돈이 썩고 있어! **Beta {beta:.2f}**로는 시장 못 이겨! 쫄보야? 당장 **레버리지** 태워!",
            f"아니 수익률이 **{avg_pnl:.2f}%**가 뭐야? 장난해? 지금 당장 공격적으로 투자해서 복구해야지! 간이 배 밖으로 나왔어?",
            f"종목이 **{stock_count}개**? 다이소 차렸어? 다 정리하고 **주도주** 하나에 몰빵해! 인생 한방이라구!"
        ]
        
        t_msgs = [
            f"자네, **보유 종목 {stock_count}개**에 **예수금 {cash_r:.1f}%**... 너무 안일해. **유비무환(有備無患)**을 모르는가? 하락장 오면 패가망신하네.",
            f"리스크 관리가 엉망이군. **MDD**가 너무 깊어질 수 있어. **과유불급(過猶不及)**이야. **[국채]**나 **[금]**을 편입해서 방어벽을 세우게.",
            f"수익률에 취해있군. **호사다마(好事多魔)**라 했어. 지금 절반 익절하고 현금 비중을 늘리게."
        ]
...
def render_native_card(d, idx=None, is_rank=False):
    ...
    with st.container(border=True):
        ...
        # Tabs with Engine Explanations
        tab1, tab2, tab3 = st.tabs(["🐹 햄찌의 잔소리", "🐯 호찌의 훈계", "📊 8대 엔진 HUD"])
        ...
        with tab3:
            ...
            h1.markdown("<div class='engine-guide'><b>🐹:</b> 진동수 안정되면 폭발!<br><b>🐯:</b> 임계점 도달 신호.</div>", unsafe_allow_html=True)
            
            h2.metric("VPIN", f"{m['vpin']:.2f}")
            h2.markdown("<div class='engine-guide'><b>🐹:</b> 높으면 설거지 당해!<br><b>🐯:</b> 정보 비대칭 위험.</div>", unsafe_allow_html=True)
            
            h3.metric("GNN", f"{m['gnn']:.2f}")
            h3.markdown("<div class='engine-guide'><b>🐹:</b> 인싸 대장주 판독기!<br><b>🐯:</b> 네트워크 중심성.</div>", unsafe_allow_html=True)
            
            st.divider()
            
            h4, h5, h6 = st.columns(3)
            h4.metric("Hawkes", f"{m['hawkes']:.2f}")
            h4.markdown("<div class='engine-guide'><b>🐹:</b> 2.0 넘으면 너도나도 매수!<br><b>🐯:</b> 자기 여진 현상 감지.</div>", unsafe_allow_html=True)
            
            h5.metric("Hurst", f"{m['hurst']:.2f}")
            h5.markdown("<div class='engine-guide'><b>🐹:</b> 0.5 넘으면 추세 굿!<br><b>🐯:</b> 시계열 지속성 판단.</div>", unsafe_allow_html=True)
            
            h6.metric("Kelly", f"{m['kelly']:.2f}")
            h6.markdown("<div class='engine-guide'><b>🐹:</b> 몰빵할 비율!<br><b>🐯:</b> 최적 자산 배분율.</div>", unsafe_allow_html=True)
