...
class SingularityEngine:
    ...
    def hamzzi_smart_nagging(self, cash, portfolio, market_data):
        ...
        title = "🐹 햄찌의 팩트 폭격"
        if cash_ratio > 70:
            msg = "사장님, 현금만 들고 있으면 인플레에 녹아요! 😱 지금 수급 들어오는 종목 좀 담으세요!"
        elif total_invest > 0 and current_val < total_invest:
            msg = "파란불이네요... 😭 '존버'는 답이 아닙니다. 추세 꺾인 건 자르고 주도주로 갈아타세요!"
        else:
            msg = "오 수익 중이시네요? 🐹 해바라기씨 사먹게 조금만 익절합시다. 욕심 부리면 탈나요!"
...
# [FUNCTION DEFINITION - MUST BE BEFORE BUTTON CALLS]
def run_market_scan():
...
if st.session_state.get('running'):
    ...
    # 3. SEPARATE VIEW
        ...
        with tab1: render_card(st.session_state.sc_list, "#FFFF00")
        with tab2: render_card(st.session_state.sw_list, "#00C9FF")
