...
# [Custom CSS: The Ultimate Dark Theme]
st.markdown("""
<style>
    /* Base App */
    ...
    /* Neon Gold Buttons */
    .stButton>button { 
        width: 100%; border-radius: 12px; font-weight: 800; height: 52px; font-size: 16px;
        background: linear-gradient(135deg, #1a1a1a 0%, #2b2b2b 100%); 
        border: 1px solid #d4af37; color: #d4af37; letter-spacing: 1px;
        box-shadow: 0 4px 15px rgba(0,0,0,0.5); transition: all 0.3s ease;
    }
    .stButton>button:hover { 
        background: linear-gradient(135deg, #d4af37 0%, #f1c40f 100%);
        color: #000; border-color: #fff;
        box-shadow: 0 0 20px rgba(212, 175, 55, 0.6); transform: translateY(-3px);
    }
    ...
    .stock-name { font-size: 24px; font-weight: 900; color: #fff; letter-spacing: -0.5px; }
    .score-badge { 
    ...
    /* Progress Bar */
    .prog-track { width: 100%; height: 10px; background: #222; margin-top: 0; }
    .prog-fill { height: 100%; transition: width 1s ease-in-out; }
    
    /* Tags & Info */
    .tag-container { padding: 15px 25px 5px 25px; display: flex; flex-wrap: wrap; gap: 8px; }
    .tag { 
        display: inline-block; padding: 5px 10px; border-radius: 6px; 
        font-size: 11px; font-weight: bold; color: #000; letter-spacing: -0.5px;
    }
    ...
    .persona-box { padding: 20px; font-size: 14px; line-height: 1.8; color: #e0e0e0; background: #1a1a1a; border-radius: 12px; margin-top: 15px; border-left-width: 4px; border-left-style: solid; }
    ...
    .t-val { font-weight: 800; font-size: 16px; margin-top: 5px; display: block; }
    
    /* Labels & HUD */
    .input-label { font-size: 13px; font-weight: bold; color: #aaa; margin-bottom: 5px; }
...
st.markdown("<h1 style='text-align: center; color: #d4af37; text-shadow: 0 0 20px rgba(212,175,55,0.4);'>🐯 Hojji & Hamzzi Quant 🐹</h1>", unsafe_allow_html=True)

# -----------------------------------------------------------------------------
# [1] INITIALIZATION (CRITICAL: STATE FIRST)
# -----------------------------------------------------------------------------
...
# Pre-load stock names to prevent NameError
...
# -----------------------------------------------------------------------------
# [2] CORE ENGINE: SINGULARITY OMEGA
# -----------------------------------------------------------------------------
class SingularityEngine:
    ...
    def generate_report(self, mode, price, m, wr, cash, current_qty, target_return):
        ...
        # --- 🐹 HAMZZI (Infinite Variety Logic) ---
        if wr >= 0.70:
            h_brief = random.choice([
                f"사장님! <b>[Hawkes {m['hawkes']:.2f}]</b> 수치 봤어?! 이건 단순 수급이 아니라 '자기 여진'에 의한 폭발적 매수세야! 🚀",
                f"대박 사건! <b>[GNN]</b> 중심성이 {m['gnn']:.2f}로 시장 돈이 다 여기로 쏠리고 있어! 지금이 기회야! 💰"
            ])
            h_act = f"쫄지마! <b>{can_buy}주</b> 시장가 매수! <b>{target:,}원</b> 뚫으면 불타기(Pyramiding) 가즈아!"
            h_why = f"변동성(Vol Surface: {m['vol_surf']:.2f})이 가파르게 상승 중이야. 베타(Beta)를 먹으려면 지금 리스크를 감수해야 해!"
        elif wr >= 0.50:
            h_brief = random.choice([
                f"음~ <b>[Hurst {m['hurst']:.2f}]</b> 추세 살아있네! 단타 치기 딱 좋은 놀이터야! 🎢",
                f"오홍? <b>[OBI]</b> 호가창 매수세가 꿈틀대고 있어. 기술적 반등 나올 자리야! ⚡"
            ])
            h_act = f"일단 <b>{int(can_buy/3)}주</b>만 정찰병 보내고, <b>{price:,}원</b> 지지하면 나머지 태워!"
            h_why = "모멘텀은 좋은데 눈치 싸움 중이야. 짧게 치고 빠지는 '게릴라 전술'이 유효해."
        else:
            h_brief = f"으악! 돔황챠!! 😱 <b>[VPIN {m['vpin']:.2f}]</b> 경고등 켜졌어! 이건 기관 형님들이 설거지하는 거야! 💣"
            h_act = "절대 매수 금지! ❌ 탈출은 지능순이야! 현금 쥐고 숨어있어!"
            h_why = "독성 매물이 쏟아지고 있어. 지금 들어가면 계좌 반토막 확정이야. 파산 확률 99%."

        # --- 🐯 HOJJI (Infinite Variety Logic) ---
        if wr >= 0.70:
            ...
            t_why = f"<b>[Omega 파동]</b>이 {m['omega']:.1f}로 안정적이야. 발 뻗고 잘 수 있는 자리일세."
        elif wr >= 0.50:
            ...
            t_brief = f"에잉 쯧쯧! 😡 <b>[독성 매물]</b>이 넘쳐나는구먼! 기초가 부실한데 어찌 오르겠나! 사상누각이야!"
            t_act = "쳐다도 보지 말게. 현금이 곧 최고의 종목이야. 🛡️"
            t_why = "스마트 머니는 이미 떠났어. 떨어지는 칼날을 맨손으로 잡으려 하지 말게."

        return {
            "prices": (price, target, stop),
            "hamzzi": {"title": "🐹 햄찌의 야수 본능", "brief": h_brief, "act": h_act, "why": h_why},
            "hojji": {"title": "🐯 호찌의 유비무환", "brief": t_brief, "act": t_act, "why": t_why}
        }
    ...
    def diagnose_portfolio(self, portfolio, cash, target_return):
        ...
        if target_return < 5: h_msg += f"목표 수익률 <b>{target_return}%</b>? 꿈이 너무 작아! 🐹 <b>[레버리지]</b> 태워서 10배는 먹어야지!<br>"
        if beta < 0.8: h_msg += f"포트폴리오가 너무 얌전해(Beta {beta:.2f})... 🐢 재미없어! <b>[주도주]</b> 좀 섞어서 화끈하게 가보자구!"
        ...
        if stock_count > 10: t_msg += f"종목이 <b>{stock_count}개</b>라니 백화점인가? 관리가 안 돼. <b>[선택과 집중]</b>이 필요하네.<br>"
...
# -----------------------------------------------------------------------------
# [4] UI RENDERER (Clean HTML Wrapper)
# -----------------------------------------------------------------------------
def render_full_card(d, idx=None, is_rank=False):
    ...
    # 1. Main Card (Using textwrap.dedent to fix HTML display issue)
    ...
    <div class='stock-card' style='border-color:{color};'>
        ...
        <div class='card-header'>
            ...
            <div class='score-badge' style='color:{color}; border-color:{color};' title='8대 엔진 종합 점수'>Score {win_pct:.1f}</div>
        </div>
    ...
    with t1:
        ...
        <div class='persona-box' style='border-left-color: #FFAA00;'>
            ...
            <div style='background:#2a2a2a; padding:15px; border-radius:10px; margin-bottom:10px;'><b>💡 행동 지침:</b> {h['act']}</div>
    ...
    with t2:
        ...
        <div class='persona-box' style='border-left-color: #FF4444;'>
            ...
            <div style='background:#2a2a2a; padding:15px; border-radius:10px; margin-bottom:10px;'><b>💡 어르신 말씀:</b> {t['act']}</div>
...
with st.expander("💰 내 자산 및 포트폴리오 설정", expanded=True):
    ...
    with c1: 
        st.markdown("<p class='input-label'>예수금 (KRW)</p>", unsafe_allow_html=True)
        st.session_state.cash = st.number_input("cash", value=st.session_state.cash, step=100000, label_visibility="collapsed")
    with c2: 
        st.markdown("<p class='input-label'>목표 수익률 (%)</p>", unsafe_allow_html=True)
    ...
    with c3:
        st.markdown("<p class='input-label'>종목 추가</p>", unsafe_allow_html=True)
        if st.button("➕ 추가"):
    ...
    if st.session_state.portfolio:
        for i, s in enumerate(st.session_state.portfolio):
            ...
            with cols[0]: 
                st.markdown(f"<p class='input-label'>종목명 {i+1}</p>", unsafe_allow_html=True)
                try: idx = stock_names.index(s['name'])
                except: idx = 0
                s['name'] = st.selectbox(f"n_{i}", stock_names, index=idx, label_visibility="collapsed")
            with cols[1]: 
                st.markdown("<p class='input-label'>평단가</p>", unsafe_allow_html=True)
            ...
            with cols[3]: 
                st.markdown("<p class='input-label'>전략</p>", unsafe_allow_html=True)
                s['strategy'] = st.selectbox(f"s_{i}", ["추세추종", "초단타"], index=0 if s['strategy']=="추세추종" else 1, label_visibility="collapsed")
            with cols[4]: 
                st.markdown("<p class='input-label'>삭제</p>", unsafe_allow_html=True)
